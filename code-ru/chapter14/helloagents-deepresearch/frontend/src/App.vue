<template>
  <main class="app-shell" :class="{ expanded: isExpanded }">
    ...
    <!-- Начальное состояние: карточка ввода по центру -->
    <div v-if="!isExpanded" class="layout layout-centered">
      <section class="panel panel-form panel-centered">
        <header class="panel-head">
          ...
          <div>
            <h1>Ассистент глубокого исследования</h1>
            <p>Многораундовый интеллектуальный поиск и резюмирование с отображением выводов и источников в реальном времени.</p>
          </div>
        </header>

        <form class="form" @submit.prevent="handleSubmit">
          <label class="field">
            <span>Тема исследования</span>
            <textarea
              v-model="form.topic"
              placeholder="Например: ключевые прорывы мультимодальных моделей в 2025 году"
              rows="4"
              required
            ></textarea>
          </label>
          ...
          <section class="options">
            <label class="field option">
              ...
              <select v-model="form.searchApi">
                <option value="">Использовать настройки бэкенда</option>
                <option
          ...
          <div class="form-actions">
            <button class="submit" type="submit" :disabled="loading">
              <span class="submit-label">
                ...
                {{ loading ? "Исследование в процессе..." : "Начать исследование" }}
              </span>
            </button>
            <button
              v-if="loading"
        ...
        <p v-else-if="loading" class="hint muted">
          Собираем данные и доказательства, прогресс отображается справа.
        </p>
      </section>
    </div>

    <!-- Полноэкранное состояние: двухколонный макет -->
    <div v-else class="layout layout-fullscreen">
      <!-- Левая колонка: информация об исследовании -->
      <aside class="sidebar">
        <div class="sidebar-header">
          ...
          <h2>🔍 Ассистент глубокого исследования</h2>
        </div>
        ...
        <div class="research-info">
          ...
          <div class="info-item" v-if="totalTasks > 0">
            <label>Прогресс исследования</label>
            <div class="progress-bar">
              <div class="progress-fill" :style="{ width: `${(completedTasks / totalTasks) * 100}%` }"></div>
            </div>
            <p class="progress-text">{{ completedTasks }} / {{ totalTasks }} задач выполнено</p>
          </div>
        </div>
      ...
      <!-- Правая колонка: результаты исследования -->
      <section
        ...
        <header class="status-bar">
          <div class="status-main">
            <div class="status-chip" :class="{ active: loading }">
              <span class="dot"></span>
              {{ loading ? "Исследование выполняется" : "Рабочий процесс завершён" }}
            </div>
            <span class="status-meta">
              Прогресс: {{ completedTasks }} / {{ totalTasks || todoTasks.length || 1 }}
              · Записей в журнале: {{ progressLogs.length }}
            </span>
          </div>
          <div class="status-controls">
            <button class="secondary-btn" @click="logsCollapsed = !logsCollapsed">
              {{ logsCollapsed ? "Развернуть журнал" : "Свернуть журнал" }}
            </button>
          </div>
        </header>
        ...
        <div class="tasks-section" v-if="todoTasks.length">
          ...
          <article class="task-detail" v-if="currentTask">
            ...
            >
              ...
              <template v-if="currentTaskSources.length">
                <ul class="sources-list">
                  ...
                  >
                    ...
                    >
                      {{ item.title || item.url || `Источник ${index + 1}` }}
                    </a>
              ...
              <p v-else class="muted">Источники отсутствуют</p>
            </section>
            ...
            >
              <h3>Резюме задачи</h3>
              <pre class="block-pre">{{ currentTaskSummary || "Информация отсутствует" }}</pre>
            </section>
            ...
            >
              <h3>Журнал вызовов инструментов</h3>
              <ul class="tool-list">
                <li
          ...
          <article class="task-detail" v-else>
            <p class="muted">Ожидание планирования или результатов выполнения задач.</p>
          </article>
        </div>
...
function parseSources(raw: string): SourceItem[] {
  ...
  for (const line of lines) {
    ...
    if (/^(Source|Источник)\s*:/.test(trimmed)) {
      flush();
    ...
    if (
      /^(Most relevant content from source|Содержимое)\s*:/.test(trimmed)
    ) {
      ensureCurrent();
    ...
    if (
      /^(Full source content limited to|Полное содержимое ограничено)\s*:/.test(trimmed)
    ) {
      ensureCurrent();
...
function formatToolParameters(parameters: Record<string, unknown>): string {
  try {
    ...
    console.warn("Не удалось отформатировать параметры инструмента", error, parameters);
...
async function copyNotePath(path: string | null | undefined) {
  ...
  try {
    ...
    progressLogs.value.push(`Путь к заметке скопирован: ${path}`);
  } catch (error) {
    console.warn("Не удалось скопировать в буфер обмена", error);
    window.prompt("Скопируйте путь к заметке", path);
    progressLogs.value.push("Скопируйте путь к заметке вручную");
  }
}
...
const handleSubmit = async () => {
  if (!form.topic.trim()) {
    error.value = "Введите тему исследования";
    return;
  }
  ...
  try {
      ...
      (event: ResearchStreamEvent) => {
        ...
        if (event.type === "todo_list") {
            ...
            return {
              ...
              intent:
                typeof item.intent === "string" && item.intent.trim()
                  ? item.intent.trim()
                  : "Изучить ключевую информацию по теме",
              query:
          ...
          if (todoTasks.value.length) {
            ...
            progressLogs.value.push("Список задач сформирован");
          } else {
            progressLogs.value.push("Список задач не сформирован, продолжаем с задачей по умолчанию");
          }
          return;
        }
        ...
        if (event.type === "task_status") {
          ...
          if (status === "in_progress") {
            ...
            progressLogs.value.push(`Начало выполнения задачи: ${task.title}`);
            ...
            progressLogs.value.push(`Задача выполнена: ${task.title}`);
            if (activeTaskId.value === task.id) {
              pulse(summaryHighlight);
              pulse(sourcesHighlight);
            }
          } else if (status === "skipped") {
            progressLogs.value.push(`Задача пропущена: ${task.title}`);
          }
          return;
        }
        ...
        if (event.type === "sources") {
          ...
          if (latestText) {
            ...
            progressLogs.value.push(`Источники задачи обновлены: ${task.title}`);
          }

          if (typeof payload.backend === "string") {
            progressLogs.value.push(
              `Используемый поисковый бэкенд: ${payload.backend}`
            );
          }
        ...
        if (event.type === "tool_call") {
          ...
          if (task) {
            ...
            const logSummary = noteId
              ? `${agent} вызвал ${tool} (задача ${task.id}, заметка ${noteId})`
              : `${agent} вызвал ${tool} (задача ${task.id})`;
            progressLogs.value.push(logSummary);
            if (activeTaskId.value === task.id) {
              pulse(toolHighlight);
            }
          } else {
            progressLogs.value.push(`${agent} вызвал ${tool}`);
          }
          return;
        }
        ...
        if (event.type === "final_report") {
          const report =
            typeof event.report === "string" && event.report.trim()
              ? event.report.trim()
              : "";
          reportMarkdown.value = report || "Генерация отчёта завершилась ошибкой: нет полезного содержимого";
          pulse(reportHighlight);
          progressLogs.value.push("Итоговый отчёт сформирован");
          return;
        }
        ...
        if (event.type === "error") {
          const detail =
            typeof event.detail === "string" && event.detail.trim()
              ? event.detail
              : "В процессе исследования произошла ошибка";
          error.value = detail;
          progressLogs.value.push("Исследование прервано из-за ошибки");
        }
      },
    ...
    if (!reportMarkdown.value) {
      reportMarkdown.value = "Отчёт не был сформирован";
    }
  } catch (err) {
    if (err instanceof DOMException && err.name === "AbortError") {
      progressLogs.value.push("Текущее исследование отменено");
    } else {
      error.value = err instanceof Error ? err.message : "Запрос завершился ошибкой";
    }
  } finally {
    loading.value = false;
...
const cancelResearch = () => {
  ...
  progressLogs.value.push("Выполняется попытка отмены текущего исследования…");
  currentController.abort();
};

const goBack = () => {
  if (loading.value) {
    return; // Нельзя вернуться во время исследования
  }
...
/* Панель результатов в полноэкранном режиме */
