"""Оркестратор, координирующий рабочий процесс глубокого исследования."""
...
class DeepResearchAgent:
    """Координатор, управляющий рабочим процессом исследования на основе TODO с использованием HelloAgents."""

    def __init__(self, config: Configuration | None = None) -> None:
        """Инициализировать координатор с конфигурацией и общими инструментами."""
        ...
        self._summarizer_factory: Callable[[], ToolAwareSimpleAgent] = lambda: self._create_tool_aware_agent(  # noqa: E501
            name="Эксперт по резюмированию задач",
    ...
    def _init_llm(self) -> HelloAgentsLLM:
        """Создать экземпляр HelloAgentsLLM в соответствии с параметрами конфигурации."""
    ...
    def _create_tool_aware_agent(self, *, name: str, system_prompt: str) -> ToolAwareSimpleAgent:
        """Создать экземпляр ToolAwareSimpleAgent с общим реестром инструментов и трекером."""
    ...
    def _set_tool_event_sink(self, sink: Callable[[dict[str, Any]], None] | None) -> None:
        """Включить или отключить немедленные обратные вызовы событий инструментов."""
        self._tool_event_sink_enabled = sink is not None
        self._tool_tracker.set_event_sink(sink)

    def run(self, topic: str) -> SummaryStateOutput:
        """Выполнить рабочий процесс исследования и вернуть итоговый отчёт."""
        state = SummaryState(research_topic=topic)
        state.todo_items = self.planner.plan_todo_list(state)
        self._drain_tool_events(state)

        if not state.todo_items:
            logger.info("Задачи не сгенерированы; используется резервная задача")
    ...
    def run_stream(self, topic: str) -> Iterator[dict[str, Any]]:
        """Выполнить рабочий процесс с передачей постепенных событий прогресса."""
        state = SummaryState(research_topic=topic)
        logger.debug("Запуск потокового исследования: topic=%s", topic)
        yield {"type": "status", "message": "Инициализация рабочего процесса исследования"}
        ...
        def worker(task: TodoItem, step: int) -> None:
            try:
                ...
                logger.exception("Выполнение задачи завершилось ошибкой", exc_info=exc)
                enqueue(
                    {
                        "type": "task_status",
                        "task_id": task.id,
    ...
    # ------------------------------------------------------------------
    # Вспомогательные методы выполнения
    # ------------------------------------------------------------------
    def _execute_task(
        self,
        state: SummaryState,
        ...
        *,
        ...
        """Выполнить поиск и резюмирование для одной задачи."""
        ...
        task.summary = summary_text.strip() if summary_text else "Информация отсутствует"
    ...
    def _drain_tool_events(
        ...
        *,
        ...
        """Прокси к общему трекеру вызовов инструментов."""
    ...
    def _tool_call_events(self) -> list[dict[str, Any]]:
        """Предоставить записанные события инструментов для устаревших интеграций."""
        return self._tool_tracker.as_dicts()

    def _serialize_task(self, task: TodoItem) -> dict[str, Any]:
        """Преобразовать датакласс задачи в сериализуемый словарь для фронтенда."""
        return {
            "id": task.id,
            "title": task.title,
            "intent": task.intent,
    ...
    def _persist_final_report(self, state: SummaryState, report: str) -> dict[str, Any] | None:
        ...
        note_title = f"Аналитический отчёт: {state.research_topic}".strip() or "Аналитический отчёт"
    ...
    def _find_existing_report_note_id(self, state: SummaryState) -> str | None:
        ...
        for event in reversed(self._tool_tracker.as_dicts()):
            ...
            if note_type != "conclusion":
                ...
                if not (isinstance(title, str) and title.startswith("Аналитический отчёт")):
                    continue
...
def run_deep_research(topic: str, config: Configuration | None = None) -> SummaryStateOutput:
    """Удобная функция, дублирующая API на основе класса."""
