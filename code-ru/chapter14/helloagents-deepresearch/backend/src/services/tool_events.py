"""Утилита для сбора и предоставления событий вызовов инструментов."""
...
@dataclass
class ToolCallEvent:
    """Внутреннее представление события вызова инструмента."""
...
class ToolCallTracker:
    """Собирает события вызовов инструментов и преобразует их в SSE-нагрузки."""
    ...
    def record(self, payload: dict[str, Any]) -> None:
        """Записать вызов инструмента моделью для логирования и отображения во фронтенде."""
        ...
        logger.info(
            "Вызов инструмента записан: agent=%s tool=%s task_id=%s note_id=%s parsed_parameters=%s",
            agent_name,
            tool_name,
            task_id,
            note_id,
    ...
    # ------------------------------------------------------------------
    # Методы извлечения событий
    # ------------------------------------------------------------------
    def drain(self, state: SummaryState, *, step: Optional[int] = None) -> list[dict[str, Any]]:
        """Извлечь необработанные события вызовов инструментов и синхронизировать note_id задач."""
    ...
    def as_dicts(self) -> list[dict[str, Any]]:
        """Предоставить снимок сырых событий для обратной совместимости."""
    ...
    def set_event_sink(self, sink: Optional[Callable[[dict[str, Any]], None]]) -> None:
        """Зарегистрировать обратный вызов для немедленных уведомлений о событиях инструментов."""
    ...
    def _attach_note_to_task(self, tasks: list[TodoItem], task_id: int, note_id: str) -> None:
        """Обновить соответствующий элемент TODO метаданными заметки."""
    ...
    def _infer_task_id(self, parameters: dict[str, Any]) -> Optional[int]:
        """Попытаться определить task_id из параметров инструмента."""
        ...
        if isinstance(title, str):
            match = re.search(r"[Зз]адача\s*(\d+)", title)
            if match:
                return int(match.group(1))
